...
const CROP_REF = {
  Cassava: {
    ...
    milestones: [
      { name: "Land preparation & planting", pct: 35, week: 1 },
      { name: "Fertiliser & weed control", pct: 30, week: 8 },
      { name: "Pest & disease management", pct: 15, week: 20 },
      { name: "Harvest & processing", pct: 20, week: 40 },
    ],
  },
  Maize: {
    costMin: 120000,
    ...
    milestones: [
      { name: "Land preparation", pct: 30, week: 1 },
      { name: "Planting & fertilising", pct: 35, week: 2 },
      { name: "Weeding & pest control", pct: 20, week: 6 },
      { name: "Harvest", pct: 15, week: 14 },
    ],
  },
  Rice: {
    costMin: 200000,
    ...
    milestones: [
      { name: "Field preparation", pct: 25, week: 1 },
      { name: "Transplanting & fertilising", pct: 35, week: 3 },
      { name: "Weeding & irrigation", pct: 25, week: 8 },
      { name: "Harvest & milling", pct: 15, week: 20 },
    ],
  },
  Tomato: {
    costMin: 300000,
    ...
    monthsMax: 4,
    maxReturn: 25,
    states: ["Kano", "Kaduna", "Plateau", "Benue", "Ogun", "Lagos"],
    milestones: [
      { name: "Nursery & land prep", pct: 30, week: 1 },
      { name: "Transplanting", pct: 25, week: 3 },
      { name: "Fertilising & staking", pct: 25, week: 5 },
      { name: "Harvest", pct: 20, week: 12 },
    ],
  },
  Yam: {
    costMin: 250000,
    ...
    monthsMax: 10,
    maxReturn: 20,
    states: ["Benue", "Taraba", "Plateau", "Nasarawa", "Oyo", "Ekiti", "Kogi"],
    milestones: [
      { name: "Land clearing & mounding", pct: 35, week: 1 },
      { name: "Planting & staking", pct: 30, week: 2 },
      { name: "Weeding & mulching", pct: 20, week: 12 },
      { name: "Harvest", pct: 15, week: 32 },
    ],
  },
  Poultry: {
    costMin: 180000,
    ...
    milestones: [
      { name: "Housing & equipment", pct: 25, week: 1 },
      { name: "Chick procurement", pct: 30, week: 2 },
      { name: "Feed & vaccination", pct: 30, week: 4 },
      { name: "Sale & processing", pct: 15, week: 10 },
    ],
  },
};
...
function LiveLocationCapture({ onLocationCapture, onClear }) {
  ...
  const handleCapture = async (e) => {
    ...
    try {
      processedFile = await compressImageIfNeeded(
        file,
        FILE_LIMITS_MB.LOCATION_PHOTO,
      );
    } catch {
      addToast(
        "Could not optimize location photo. Please try another image.",
        "error",
      );
      return;
    }
  ...
  return (
    ...
    >
      ...
      >
        Live Farm Location Verification
      </h4>
      <p
        style={{
          fontSize: "12px",
      ...
      >
        Take a live picture at the farm to verify its GPS coordinates. This
        anchors your farm on AgriFlow. (Will not be shown to investors)
      </p>
      <p
        style={{
          fontSize: "12px",
      ...
      >
        Maximum location photo size: {FILE_LIMITS_MB.LOCATION_PHOTO}MB
      </p>
        ...
        >
          ...
          >
            {location ? (
              ...
              >
                <Icon name="milestones" size={14} /> Location captured (
...
function FarmCreationForm({ onDone, continueFarm = null }) {
  ...
  const { getRootProps, getInputProps } = useDropzone({
    ...
    onDrop: async (files) => {
      ...
      if (acceptedPhotos.length) {
        setData((prev) => ({
          ...prev,
          photos: [...prev.photos, ...acceptedPhotos],
        }));
      }
    ...
    onDropRejected: (rejections) => {
      addToast(
        "Some files were rejected. Please upload only valid image files.",
        "error",
      );
    },
  });
  ...
  useEffect(() => {
    ...
    setData((prev) => ({
      ...
      totalBudget: continueFarm.total_budget
        ? String(continueFarm.total_budget)
        : "",
      startDate: continueFarm.start_date || "",
      endDate: continueFarm.harvest_date || "",
      expectedYield: continueFarm.expected_yield
        ? String(continueFarm.expected_yield)
        : "",
      salePrice: continueFarm.sale_price_per_unit
        ? String(continueFarm.sale_price_per_unit)
        : "",
      returnRate: continueFarm.return_rate
        ? String(continueFarm.return_rate * 100)
        : "",
      photos: [],
      location: null,
      locationPhoto: null,
  ...
  const stageTotal = data.stages.reduce(
    (s, st) => s + (parseFloat(st.amount) || 0),
    0,
  );
  const budgetWarn = budgetMax && totalBudget > budgetMax;
  const stageMismatch =
    totalBudget > 0 && Math.abs(stageTotal - totalBudget) > totalBudget * 0.05;
  ...
  useEffect(() => {
    ...
    const generated = ref.milestones.map((m) => ({
      name: m.name,
      amount: Math.round((totalBudget * m.pct) / 100).toString(),
      locked: true,
    }));
  ...
  const handleSubmit = async () => {
    if (!selectedCropObj)
      return addToast("Please select a valid crop", "error");
    if (!data.location || !data.locationPhoto)
      return addToast("Please capture farm location and photo", "error");
    ...
    if (!data.name || data.name.trim().length < 3)
      return addToast("Farm name must be at least 3 characters", "error");
    if (!data.state) return addToast("Please select a state", "error");
    if (!data.lga) return addToast("Please enter an LGA", "error");
    if (isNaN(farmSizeHa) || farmSizeHa <= 0)
      return addToast("Please enter a valid farm size", "error");
    if (!data.description || data.description.trim().length < 20)
      return addToast("Description must be at least 20 characters", "error");
    if (isNaN(totalBudget) || totalBudget <= 0)
      return addToast("Please enter a valid total budget", "error");
    if (isNaN(expectedYield) || expectedYield <= 0)
      return addToast("Please enter a valid expected yield", "error");
    if (isNaN(salePricePerUnit) || salePricePerUnit <= 0)
      return addToast("Please enter a valid sale price per unit", "error");
    if (isNaN(returnRate) || returnRate <= 0)
      return addToast("Please enter a valid return rate", "error");
    ...
    if (
      data.locationPhoto &&
      data.locationPhoto.size > mbToBytes(FILE_LIMITS_MB.LOCATION_PHOTO)
    ) {
      return addToast(
    ...
    if (oversizedDisplayPhoto) {
      return addToast(
        `Each display photo must be ${FILE_LIMITS_MB.DISPLAY_PHOTO}MB or less`,
        "error",
      );
    }
    ...
    <div className="farm-form card">
      <div className="form-steps">
        {steps.map((s, i) => (
          ...
          >
            <span
              className={`fstep${step === i + 1 ? " act" : step > i + 1 ? " done" : ""}`}
            >
              {step > i + 1 ? "✓" : i + 1}
            </span>
            <span
        ...
        <div className="fsec">
          ...
          <div className="form-group">
            ...
            <input
              ...
              placeholder="e.g. Oduya Cassava Farm"
            />
          </div>
          <div className="frow">
            <div className="form-group">
              ...
              <select
                ...
                value={data.crop}
                onChange={(e) => u("crop", e.target.value)}
              >
            ...
            <div className="form-group">
              ...
              >
                ...
                {(ref ? ref.states : nigeriaStates).map((s) => (
              ...
              {ref && data.state && !ref.states.includes(data.state) && (
                ...
                >
                  <Icon name="alert" size={12} /> {data.crop} is not commonly
                  grown in {data.state}
                </p>
              )}
            </div>
          ...
          <div className="frow">
            <div className="form-group">
              ...
              <input
                className="form-input"
                value={data.lga}
                onChange={(e) => u("lga", e.target.value)}
          ...
          {ref && sz > 0 && (
            ...
            >
              ...
              >
                <Icon name="sprout" size={24} /> AgriFlow suggests for{" "}
                {data.crop} · {sz} ha
              </p>
              <div
                style={{
              ...
              >
                {[
                  [
                    "Budget range",
                    `${formatCurrency(ref.costMin * sz)} – ${formatCurrency(ref.costMax * sz)}`,
                  ],
                  [
                    "Expected yield",
                    `${ref.yieldMin * sz} – ${ref.yieldMax * sz} ${ref.unit}`,
                  ],
                  [
                    "Revenue estimate",
                    `${formatCurrency(revMin)} – ${formatCurrency(revMax)}`,
                  ],
                  [
                    "Growing period",
                    `${ref.monthsMin} – ${ref.monthsMax} months`,
                  ],
          ...
          <div className="form-group">
            ...
            <textarea
              ...
              placeholder="Describe your farm for investors…"
            />
          </div>
        </div>
      )}
        ...
        <div className="fsec">
          ...
          {ref && sz > 0 && (
            ...
            >
              Reference budget:{" "}
              <strong>
                {formatCurrency(ref.costMin * sz)} –{" "}
                {formatCurrency(ref.costMax * sz)}
              </strong>{" "}
              for {sz}ha of {data.crop}
            </div>
          )}
          <div className="form-group">
              ...
              onChange={(e) => {
                ...
                if (ref && data.stages.length > 0) {
                  ...
                  setData((p) => ({
                    ...p,
                    totalBudget: v,
                    stages: ref.milestones.map((m) => ({
                      name: m.name,
                      amount: Math.round((b * m.pct) / 100).toString(),
                      locked: true,
                    })),
                  }));
                }
            ...
            {budgetWarn && (
              ...
              >
                <Icon name="alert" size={12} /> Exceeds reference max by &gt;20%
                for this crop and farm size
              </p>
            )}
          </div>
          ...
          {ref ? (
            <>
              ...
              >
                Stages are pre-set from the {data.crop} template. Amounts adjust
                automatically.
              </p>
              <div
                style={{
              ...
              >
                {data.stages.map((st, i) => (
                  ...
                  >
                    ...
                    >
                      ₦{(parseInt(st.amount) || 0).toLocaleString()}
                    </span>
                    <span
                      style={{
                        fontSize: "11px",
            ...
            <>
                ...
                {data.stages.map((st, i) => (
                  ...
                  >
                    <input
                      className="form-input"
                      value={st.name}
                      onChange={(e) => us(i, "name", e.target.value)}
                      placeholder="Stage"
                      style={{ flex: 1 }}
                    />
                    <input
                      ...
                      type="number"
                      value={st.amount}
                      onChange={(e) => us(i, "amount", e.target.value)}
                      placeholder="Amount"
                      style={{ width: "140px" }}
                    />
                    <button
              ...
              {stageMismatch && (
                ...
                >
                  <Icon name="alert" size={12} /> Stage totals (₦
                  {stageTotal.toLocaleString()}) don't match budget (₦
            ...
            {data.stages
              ...
              .map((s, i) => {
                const pct =
                  totalBudget > 0
                    ? (
                        ((parseFloat(s.amount) || 0) / totalBudget) *
                        100
                      ).toFixed(1)
                    : 0;
                return (
      ...
      {step === 3 && (
        <div className="fsec">
          ...
          {ref && (
            ...
            >
              Growing period for {data.crop}:{" "}
              <strong>
                {ref.monthsMin}–{ref.monthsMax} months
              </strong>{" "}
              · Max return rate: <strong>{ref.maxReturn}%</strong>
            </div>
          )}
          <div className="frow">
            <div className="form-group">
            ...
            <div className="form-group">
              <label className="form-label">
                ...
                {ref && sz > 0 && (
                  ...
                  >
                    ref: {ref.yieldMin * sz}–{ref.yieldMax * sz}
                  </span>
                )}
              </label>
              <input
                ...
                onChange={(e) => u("expectedYield", e.target.value)}
                placeholder={
                  ref && sz
                    ? `e.g. ${(((ref.yieldMin + ref.yieldMax) / 2) * sz).toFixed(1)}`
                    : "e.g. 4.2"
                }
              />
              {ref &&
                sz > 0 &&
                parseFloat(data.expectedYield) > ref.yieldMax * sz * 1.3 && (
                  <p
                    style={{
                      fontSize: "12px",
                      color: "var(--color-danger)",
                  ...
                  >
                    <Icon name="alert" size={12} /> Exceeds reference max yield
                    for this farm size
                  </p>
                )}
            </div>
            <div className="form-group">
              <label className="form-label">
                ...
                {ref && (
                  ...
                  >
                    ref: ₦{(ref.priceMin / 1000).toFixed(0)}k–₦
                    {(ref.priceMax / 1000).toFixed(0)}k
                  </span>
                )}
              </label>
              <input
                ...
                onChange={(e) => u("salePrice", e.target.value)}
                placeholder={
                  ref
                    ? `e.g. ${Math.round((ref.priceMin + ref.priceMax) / 2)}`
                    : "e.g. 150000"
                }
              />
            </div>
          ...
          <div className="form-group">
            <label className="form-label">
              ...
              {ref && (
                ...
                >
                  max {ref.maxReturn}% for {data.crop}
                </span>
              )}
            </label>
            <input
              ...
              onChange={(e) => u("returnRate", e.target.value)}
              placeholder={`e.g. ${ref?.maxReturn || 15}`}
            />
            {ref && parseFloat(data.returnRate) > ref.maxReturn && (
              ...
              >
                <Icon name="alert" size={12} /> Exceeds the maximum allowed
                return for {data.crop} ({ref.maxReturn}%). AgriFlow only allows
                data-backed returns.
              </p>
            )}
          </div>
        ...
        <div className="fsec">
          ...
          >
            {[
              ...
              ["Harvest", data.endDate],
              ["Yield", `${data.expectedYield} ${ref?.unit || "tons"}`],
          ...
          >
            Please confirm all text details are correct before proceeding to
            photo uploads and GPS anchoring.
          </div>
        </div>
      )}
      ...
      {step === 5 && (
        <div className="fsec">
          ...
          {continueFarm && (
            ...
            >
              Continuing registration for <strong>{continueFarm.name}</strong>.
              Complete this upload step to submit the farm for review.
            </div>
          )}
          <p
            style={{
          ...
          >
            Final step: Anchor your farm with a live location photo and upload
            display pictures for investors.
          </p>

          <LiveLocationCapture
            onLocationCapture={(loc, photo) => {
              u("location", loc);
              u("locationPhoto", photo);
            }}
            onClear={() => {
              u("location", null);
              u("locationPhoto", null);
            }}
          />
          ...
          <div className="form-group" style={{ marginTop: "8px" }}>
            ...
            >
              Maximum display photo size: {FILE_LIMITS_MB.DISPLAY_PHOTO}MB each
            </p>
            <div {...getRootProps()} className="dropzone">
              ...
              >
                <Icon name="add" size={16} /> Drag photos here or click to
                browse
              </span>
            </div>
          ...
          >
            ...
            >
              By submitting, you certify that the location photo was taken
              on-site. AgriFlow uses Smart Guard GPS validation to detect fraud.
            </p>
          </div>
...
function ProofUpload({ milestone, onSuccess }) {
  ...
  const handleCapture = async (e) => {
    ...
    try {
      processedFile = await compressImageIfNeeded(
        file,
        FILE_LIMITS_MB.MILESTONE_PHOTO,
      );
    } catch {
      addToast(
        "Could not optimize proof photo. Please try another image.",
        "error",
      );
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      addToast(
        "Proof submitted!",
        "success",
        "Photo and location logged. Admin will review shortly.",
      );
      ...
      addToast(
        "Upload failed",
        "error",
        err.response?.data?.detail || "Could not upload proof",
...
function MilestonesTab() {
  ...
  const [selectedFarmId, setSelectedFarmId] = useState(
    initialFarmId && initialFarmId !== "null" && initialFarmId !== "undefined"
      ? initialFarmId
      : null,
  );
    ...
    return (
      <div>
        ...
        <div className="card" style={{ padding: "40px", textAlign: "center" }}>
          ...
          >
            You don't have any approved farms with milestones yet. Once your
            farm is approved, milestones will appear here.
          </p>
        </div>
      </div>
    );
  ...
  const selectedFarm = selectedFarmId
    ? activeFarms.find((f) => f.id === selectedFarmId)
    : null;
  ...
  if (!selectedFarmId || !selectedFarm) {
    return (
      <div>
        ...
        >
          Select a farm to view and manage its milestones.
        </p>
    ...
    <div>
      ...
      >
        ...
        <div>
          ...
          <p style={{ color: "var(--color-text-secondary)", fontSize: "14px" }}>
            {selectedFarm.name} · {selectedFarm.crop_name}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
        {fetchingMilestones && milestones.length === 0 ? (
          ...
          >
            No milestones found for this farm.
          </div>
        ) : (
          milestones.map((m) => (
            <div
            ...
            >
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  >
                    ...
                    <span
                      className={`badge badge-${
                        ["verified", "disbursed"].includes(m.status)
                          ? "active"
                          : [
                                "under_review",
                                "pending_proof",
                  ...
                  >
                    Due:{" "}
                    {/* Assuming a due date exists, otherwise use expected week */}
                    {selectedFarm.start_date
                      ? new Date(
                          new Date(selectedFarm.start_date).getTime() +
                            m.expected_week * 7 * 24 * 60 * 60 * 1000,
                        ).toLocaleDateString("en-NG", {
                          day: "numeric",
                          month: "short",
                          year: "numeric",
                        })
                      : `Week ${m.expected_week}`}
                  </p>
                  ...
                  >
                    {m.status === "verified" && (
                      ...
                      >
                        <Icon name="milestones" size={16} /> Approved{" "}
                        <span
                          style={{
                            opacity: 0.6,
                            fontSize: "11px",
                            borderLeft: "1px solid rgba(26, 93, 59, 0.3)",
                            paddingLeft: "8px",
                            marginLeft: "2px",
                          }}
                        >
              ...
              {m.status === "rejected" && (
                ...
                >
                  ...
                  {m.rejection_reason ||
                    "Proof did not meet requirements. Please check your photo and coordinates and resubmit."}
                </div>
              )}
              ...
              {m.status === "locked" && (
                ...
                >
                  <span style={{ fontSize: "16px" }}>🔒</span> This stage is
                  locked. It will be available once previous milestones are
                  verified and funds are disbursed.
                </div>
              )}
            </div>
          ))
...
function HarvestTab() {
  ...
  const { getRootProps, getInputProps } = useDropzone({
    ...
    onDropRejected: () => {
      addToast(
        "Some files were rejected. Please upload image files only.",
        "error",
      );
    },
  });
  ...
  const variance =
    ay && expectedYield
      ? (((parseFloat(ay) - expectedYield) / expectedYield) * 100).toFixed(1)
      : null;

  const verifiedReport = selectedFarm?.harvest_reports?.find(
    (r) => r.status === "verified",
  );
  const submittedReport = selectedFarm?.harvest_reports?.find(
      ...
      const fetchRepayment = async () => {
        ...
        try {
          const res = await api.get(
            `/harvest/farms/${selectedFarmId}/repayment/details`,
          );
  ...
  const handleRepay = async () => {
    ...
    try {
      const res = await api.post(
        `/harvest/farms/${selectedFarmId}/repayment/initiate`,
      );
      const {
        txn_ref,
        amount_kobo,
      ...
      addToast(
        err.response?.data?.detail || "Failed to initiate repayment",
        "error",
      );
      setRepaymentLoading(false);
    }
  ...
  const verifyRepayment = async (txnRef) => {
    setIsVerifying(true);
    try {
      const res = await api.post("/harvest/repayment/verify", {
        txn_ref: txnRef,
      });
      if (res.data.data.status === "confirmed") {
        addToast(
          "Repayment Confirmed!",
          "success",
          "Investor payouts have been generated.",
        );
        // Refresh farms
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.post(
        `/harvest/farms/${selectedFarmId}/harvest-report`,
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        },
      ...
      addToast(
        err.response?.data?.message || "Failed to submit harvest report",
        "error",
      );
    } finally {
      setIsSubmitting(false);
  ...
  if (readyFarms.length === 0) {
    return (
      <div>
        ...
        >
          Submit reports once your farm cycle is complete.
        </p>
        ...
        >
          ...
          <div style={{ maxWidth: "400px" }}>
            ...
            >
              Farms appear here automatically once **all** funding milestones
              have been verified and disbursed. Check your [Milestones](#) tab
              to track progress.
            </p>
          </div>
        </div>
  ...
  if (!selectedFarmId) {
    return (
      <div>
        ...
        >
          Track your harvest cycle and settle investor returns.
        </p>
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <strong>
                      {farm.expected_yield}{" "}
                      {farm.crop_name === "Poultry" ? "birds" : "tons"}
                    </strong>
                  </div>
                  <div
        ...
        {verifiedReport ? (
          ...
          >
            ...
            >
              ...
              >
                Your harvest report has been verified by the platform. You are
                now required to settle the total investment plus the agreed ROI.
              </p>
            </div>
            ...
            >
              ...
              >
                ...
                <strong>
                  ₦{verifiedReport.admin_confirmed_sales?.toLocaleString()}
                </strong>
              </div>
            </div>
            ...
            {repaymentDetails?.is_test_mode_scaled && (
              ...
              >
                ...
                >
                  Your total settlement of{" "}
                  <strong>
                    ₦{repaymentDetails.total_repayment.toLocaleString()}
                  </strong>{" "}
                  exceeds the sandbox limit. The gateway will process{" "}
                  <strong>
                    ₦{repaymentDetails.scaled_repayment.toLocaleString()}
                  </strong>{" "}
                  ({repaymentDetails.scale_factor}x scale), but your full
                  repayment will be recorded.
                </p>
              </div>
            )}
            ...
            >
              ...
              >
                <span style={{ color: "var(--color-text-secondary)" }}>
                  Principal (Raised Amount)
                </span>
                <strong className="text-mono">
                  {formatCurrency(repaymentDetails?.principal || 0)}
                </strong>
              </div>
              <div
                style={{
              ...
              >
                <span style={{ color: "var(--color-text-secondary)" }}>
                  Interest / ROI (
                  {((repaymentDetails?.roi_rate || 0) * 100).toFixed(1)}%)
                </span>
                <strong
                  className="text-mono"
                  style={{ color: "var(--color-primary)" }}
                >
                  +{formatCurrency(repaymentDetails?.gain || 0)}
                </strong>
              </div>
              <div
                style={{
              ...
              >
                ...
                >
                  {formatCurrency(repaymentDetails?.total_repayment || 0)}
                </strong>
              </div>
            </div>
            ...
            {isRepaid ? (
              ...
              >
                <Icon name="milestones" size={18} /> Repayment Confirmed &
                Settled
              </div>
            ) : (
              <div
              ...
              >
                ...
                >
                  {repaymentLoading ? (
                    ...
                    `Pay ${formatCurrency(repaymentDetails?.total_repayment || 0)}`
                  )}
                </button>
                <p
                  style={{
                ...
                >
                  Securely processed by Interswitch. Immediate investor payouts
                  will be triggered.
                </p>
              </div>
            )}
          ...
          >
            ...
            >
              ...
              >
                Your harvest report has been submitted successfully. Our admin
                team will verify your sales data and confirm the final ROI
                before you can proceed with repayment.
              </p>
            </div>
            ...
            >
                ...
                <div style={{ display: "grid", gap: "12px" }}>
                  ...
                  >
                    ...
                    <strong style={{ fontSize: "15px" }}>
                      {submittedReport.actual_yield} units
                    </strong>
                  </div>
                  <div
                    style={{
                  ...
                  >
                    ...
                    >
                      ₦{submittedReport.total_sales_declared.toLocaleString()}
                    </strong>
                  </div>
                  <div
                    style={{
              ...
              <div>
                ...
                >
                  Sales Evidence (
                  {submittedReport.payment_evidence_urls?.length || 0})
                </div>
                <div
                  style={{
                    display: "grid",
          ...
          <>
            <div
              style={{
                padding: "16px",
                background: "rgba(57, 102, 57, 0.05)",
                borderRadius: "12px",
                border: "1px solid var(--color-primary-light)",
                fontSize: "13px",
                lineHeight: 1.5,
            ...
            >
              ...
              <strong>Final Verification Step:</strong> Please provide accurate
              yield and sales data. This will be verified against your uploaded
              evidence before ROI is settled with investors.
            </div>
            ...
            >
              <div className="form-group">
                <label className="form-label">
                  Actual Yield (
                  {selectedFarm?.crop_name === "Poultry" ? "birds" : "tons"})
                </label>
                <input
                  className="form-input"
                  type="number"
                  ...
                  placeholder={`Expected: ${expectedYield}`}
                />
              </div>
              <div className="form-group">
            ...
            <div className="form-group">
              ...
              >
                ...
                <span>
                  Expected:{" "}
                  <strong>
                    ₦{selectedFarm?.expected_revenue?.toLocaleString()}
                  </strong>
                </span>
              </div>
            </div>
            ...
            <div className="form-group">
              <label className="form-label">
                Sales Evidence (Receipts/Bank Alerts){" "}
              ...
              >
                ...
                {evidence.length > 0 ? (
                  ...
                  <div>
                    ...
                    >
                      Click to upload payment photos
                    </span>
                    <span
                      style={{
                        fontSize: "12px",
                    ...
                    >
                      PNG or JPG up to {FILE_LIMITS_MB.PAYMENT_PHOTO}MB each
                    </span>
                  </div>
                )}
              </div>
            ...
            {variance !== null && (
              ...
              >
                <div style={{ display: "flex", gap: "20px" }}>
                  <span>
                    Expected: <strong>{expectedYield}</strong>
                  </span>
                  <span>
                    Reported: <strong>{ay}</strong>
                  </span>
                </div>
                <div
                  style={{
...
export default function FarmerDashboard() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        await Promise.allSettled([fetchProfile(), fetchFarms(), fetchBanks()]);
      } catch (err) {
  ...
  const draftFarmToContinue =
    tab === "add" && draftFarmId
      ? farms.find(
          (farm) => farm.id === draftFarmId && farm.farm_status === "draft",
        )
      : null;
  ...
  const handleSavePayout = async () => {
    ...
    try {
      ...
      addToast(
        err.response?.data?.message || "Failed to save payout details",
        "error",
      );
    }
  };
          ...
          <>
            ...
            >
              ...
              {!user?.bvn_verified ? (
                <div style={{ textAlign: "center", padding: "12px 0" }}>
                  ...
                  >
                    to receive your score and start listing farms
                  </p>
                </div>
              ) : (
                <div
                ...
                >
                  ...
                  >
                    Complete more farms to earn additional points. All farmers
                    are{" "}
                    <span
                      style={{
                        color: "var(--color-text-primary)",
            ...
            >
              ...
              <button
                className="btn btn-solid btn-sm"
                onClick={() =>
                  kycComplete ? handleTabChange("add") : setIsKycOpen(true)
                }
              >
                + Add Farm
              </button>
            ...
            {displayFarms.some((f) => f.status === "deadline_passed") &&
              displayFarms
                .filter((f) => f.status === "deadline_passed")
                .map((farm) => (
                  <div
                    key={`dp-${farm.id}`}
                    style={{
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          Your farm "{farm.name}" did not reach its funding goal
                          of ₦{(farm.total_budget || 0).toLocaleString()}
                        </p>
                        <p
                          style={{
                            fontSize: "13px",
                        ...
                        >
                          You raised ₦
                          {(farm.amount_raised || 0).toLocaleString()} (
                          {farm.total_budget
                            ? Math.round(
                                (farm.amount_raised / farm.total_budget) * 100,
                              )
                            : 0}
                          %). You have <strong>47 hours</strong> to decide:
                        </p>
                      </div>
                    </div>
                    <div
                      ...
                      <button
                        ...
                        onClick={() =>
                          addToast(
                            "Deadline extended!",
                            "success",
                            "Investors have been notified of the new window.",
                          )
                        }
                      >
                        Extend Deadline
                      ...
                      <button
                        ...
                        onClick={() =>
                          addToast(
                            "Farm cancelled.",
                            "error",
                            "All investors will be refunded automatically.",
                          )
                        }
                      >
                        Cancel &amp; Refund Investors
                      </button>
                    </div>
                    <p
                      style={{
                    ...
                    >
                      If you do not respond, investors will be automatically
                      refunded after the 48-hour window closes.
                    </p>
                  </div>
                ))}
            ...
            {displayFarms.length === 0 ? (
              <EmptyState
                title="No farms yet"
                description="Create your first farm listing."
                action={() => handleTabChange("add")}
              ...
              >
                {paginatedFarms.map((farm) => {
                  ...
                  switch (farm.farm_status) {
                    ...
                    case "cancelled":
                      statusBadge = "danger";
                      statusLabel =
                        farm.farm_status.charAt(0).toUpperCase() +
                        farm.farm_status.slice(1);
                      break;
                    default:
                      statusLabel =
                        farm.farm_status?.charAt(0).toUpperCase() +
                  ...
                  const hasUploadableMilestone =
                    Array.isArray(farm.milestones) &&
                    farm.milestones.some((m) =>
                      ["pending_proof", "rejected"].includes(m.status),
                    );
                  ...
                  return (
                    <div
                      ...
                      style={{
                        padding: "20px 24px",
                        opacity: ["cancelled", "rejected"].includes(
                          farm.farm_status,
                        )
                          ? 0.7
                          : 1,
                      ...
                      >
                        ...
                        <div style={{ display: "flex", gap: "8px" }}>
                          ...
                          {isReadyForHarvest ? (
                            <button
                              className="btn btn-solid btn-sm"
                              onClick={() => handleTabChange("harvest")}
                            >
                              Report Harvest
                            </button>
                          ) : (
                            hasUploadableMilestone &&
                            !isIncompleteRegistration &&
                            ![
                              "deadline_passed",
                              "cancelled",
                ...
                <Pagination
                  ...
                  totalPages={Math.ceil(displayFarms.length / ITEMS_PER_PAGE)}
                  onPageChange={setFarmsPage}
                />
              </div>
            )}
          </>
          ...
          <div>
            ...
            {!kycComplete ? (
              ...
              >
                ...
                >
                  You must complete your BVN and Bank Account verification in
                  Settings before creating a farm.
                </p>
                <button
                  className="btn btn-solid"
              ...
              >
                ...
                >
                  Our team will review your farm listing within 24 hours.
                </p>
                <button
                  className="btn btn-solid"
              ...
              <FarmCreationForm
                continueFarm={draftFarmToContinue}
                onDone={() => {
                  setDone(true);
                  fetchFarms();
                }}
        ...
        {tab === "explore" && (
          <div>
            ...
            >
              ...
              >
                Seeing how other farmers present their projects? Use what you
                learn when creating your own.
              </span>
              <button
                className="btn btn-ghost btn-sm"
                onClick={() => setTab("add")}
            ...
            >
              <p style={{ marginBottom: "16px" }}>
                Browse active farms on the platform.
              </p>
              <Link to="/farms" className="btn btn-solid">
                Open Farm Marketplace →
              </Link>
            </div>
          </div>
        )}
          ...
          <div>
            ...
            >
              ...
              >
                ...
                <div className="form-group">
                  ...
                  <textarea
                    className="form-input form-textarea"
                    rows={3}
                    placeholder="Tell investors about your farming experience…"
                  />
                </div>
              ...
              >
                ...
                >
                  Complete your verification to build trust with investors and
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      alignItems: "center",
                      padding: "12px",
                      background: user?.bank_verified
                        ? "rgba(16, 185, 129, 0.05)"
                        : "var(--color-surface)",
                      borderRadius: "8px",
                      border: user?.bank_verified
                        ? "1px solid rgba(16, 185, 129, 0.2)"
                        : "1px solid var(--color-border)",
                    }}
                  >
              ...
              >
                ...
                >
                  Proceeds from your farm projects are automatically disbursed
                  to this account. To change these details, update your bank
                  verification.
                </p>
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ fontWeight: 600 }}>
                        {banks.find((b) => b.code === user.bank_code)?.name ||
                          "Verified Bank"}
                      </div>
                    </div>
